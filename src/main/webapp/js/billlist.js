...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"billview?id="+ obj.attr("billid");
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/jsp/upBillView.action?smbmsBill.id="+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
